    ...
    public ResponseEntity<?> deleteWarehouse(@AuthenticationPrincipal UserDetails userDetails,
        ...
        for (UserRole role : roles) {
            if (user.getRole() == role) {
                ...
                return ResponseEntity.ok().body(Collections.singletonMap("success", "Successfully deleted"));
            }
        }
        ...
        try {
            ...
            if (body.get("employee-id") != null && !body.get("employee-id").isEmpty()) {
                ...
                if (existingWarehouse != null) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Collections.singletonMap("error", "Employee already has a warehouse"));
                }

                warehouse.setManager(employee);
            }
            warehouseService.save(warehouse);
            return ResponseEntity.ok().body(Collections.singletonMap("success", "Successfully updated"));
        ...
        try {
            ...
            if (employeeService.findByUser(user).isPresent()) {
                return ResponseEntity.badRequest().body(
                        Collections.singletonMap("error", "User is already an employee"));
            }

            if (user.getRole() == UserRole.SUPPLIER) {
                return ResponseEntity.badRequest().body(
                        Collections.singletonMap("error", "User is already a supplier"));
            }

            if (request.iin().length() != 12) {
                return ResponseEntity.badRequest().body(
                        Collections.singletonMap("error", "Invalid iin"));
            }

            if (employeeService.findByIin(request.iin()).isPresent()) {
                return ResponseEntity.badRequest().body(
                        Collections.singletonMap("error", "Employee with this iin already exists"));
            }
    ...
    public ResponseEntity<?> deleteEmployee(
        ...
        try {
            ...
            return ResponseEntity.ok().body(
                    Collections.singletonMap("success", "Employee deleted successfully"));
        } catch (AuthorizationException e) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
        }
    }
